      <div class="menu">
        <ul>
          <li class="1"><a href="index.php" class="yellow">Home</a></li>
          <li class="2"><a href="free-hosting.php" class="green">Free Hosting</a></li>
          <li class="3"><a href="premium-hosting.php" class="red">Premium Hosting</a></li>
          <!-- <li class="4"><a href="domains.php" class="blue">Domain Names</a></li> -->
          <li class="4"><a href="whyus.php" class="purple">Why Us</a></li>
          <li class="5"><a target="_blank" href="<? echo $supporturl;?>" class="grey">Support</a></li>
		      <li class="6"><a target="_blank" href="http://cpanel.<? echo "$yourdomain" ;?>" class="red">cPanel</a></li>
          <li class="1"><a href="../index.php" class="yellow">切换中文</a></li>
        </ul>
      </div>